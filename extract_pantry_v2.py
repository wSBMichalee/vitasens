...
body_error = get_body(code_error)
...
body_promo = get_body(code_promo)
...
body_sb = get_body(code_sb)
body_sb = body_sb.replace('_searchController', 'controller')
body_sb = body_sb.replace('_searchQuery', 'searchQuery')
body_sb = re.sub(r'onChanged:\s*\(v\)\s*\{\s*setState\(\(\)\s*=>\s*searchQuery\s*=\s*v\);\s*\},', 'onChanged: onChanged,', body_sb)
...
body_fc = get_body(code_fc)
body_fc = body_fc.replace("state.selectedFilter", "selectedFilter")
...
print("Extraction script v2 completed.")
