...
file_path = 'lib/features/macros/presentation/screens/progress_screen.dart'
widget_dir = 'lib/features/macros/presentation/widgets/progress/'
...
classes = [
    ('_WeeklyConsistencyCard', 'WeeklyConsistencyCard', 'weekly_consistency_card.dart', []),
    ('_TodaySummaryHeader', 'TodaySummaryHeader', 'today_summary_header.dart', []),
    ('_MacrosListCard', 'MacrosListCard', 'macros_list_card.dart', []),
    ('_MacroRow', 'MacroRow', 'macros_list_card.dart', []),
    ('_RecentMealsList', 'RecentMealsList', 'recent_meals_list.dart', [
...
for filename, data in file_contents.items():
    ...
    # Update cross-references inside the extracted widgets
    full_code = full_code.replace('_MacroRow', 'MacroRow')
    with open(os.path.join(widget_dir, filename), 'w', encoding='utf-8') as f:
        f.write(full_code)

text = text.replace('_WeeklyConsistencyCard', 'WeeklyConsistencyCard')
text = text.replace('_TodaySummaryHeader', 'TodaySummaryHeader')
text = text.replace('_MacrosListCard', 'MacrosListCard')
text = text.replace('_MacroRow', 'MacroRow')
text = text.replace('_RecentMealsList', 'RecentMealsList')

new_imports = '''import '../widgets/progress/weekly_consistency_card.dart';
